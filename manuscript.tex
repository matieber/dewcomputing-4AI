...
services are provided by dew servers with resources that can be accessed
within the scope of a local area network. By contrast, the global
operation mode involves reaching cloud servers to mitigate limitations
of resource-constrained local nodes. Rendered services can be storage
or computing capabilities. In this work, we focus on evaluating collaboration
among consumer devices to perform cloud-agnostic computing services
for realtime execution of computer vision tasks using DL. We evaluate
this with a series of experiments that comprise real smartphones and
SBCs that exclusively relying on its local operation mode capabilities,
collaborate in performing inferences using real DL models -- particularly
CNN -- deployed in targeted instances over data streams of different
characteristics, i.e., workloads.
...
architectural assumptions we made in our exploration tests. In Section~\ref{sec:Empirical-evaluation}
we explain details concerning workloads, node setups and metrics,
as well as, results obtained in our empirical evaluation. Lastly,
Section~\ref{sec:Conclusions-and-future} presents the conclusions
...
}\subfloat[Cow BCS CNN model benchmarking\label{fig:Cow-BCS-Benchmarking}]{\noindent\raggedleft{}\includegraphics[bb=0bp 50bp 950bp 600bp,clip,scale=0.35]{figures/BCS_xnnpack_true}}\\
...
}\subfloat[YoloV4 tiny CNN model benchmarking\label{fig:YoloV4-benchmarking}]{\includegraphics[bb=0bp 50bp 1050bp 610bp,clip,scale=0.35]{figures/yolov4_tiny_416_tflite_xnnpack_false}
...
\includegraphics[bb=0bp 50bp 1300bp 610bp,clip,scale=0.35]{figures/foot_xnnpack_false}
\par\end{centering}
}\\
